...
:: Check if snapkey-agent.exe exists
...
:: Verify backend connectivity (optional check)
...
if %errorlevel% equ 0 (
    ...
    echo ⚠️  Could not verify backend connectivity - may need internet connection
)
echo.
...
if %errorlevel% equ 0 (
    ...
    echo.
    echo ⚠️  WARNING: Could not create Task Scheduler entry.
